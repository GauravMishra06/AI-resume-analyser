...
const InsightsPage: React.FC = () => {
    ...
    const buildSummaryText = () => {
        const lines: string[] = [
            ...
            `Score:       ${score}/10 — ${suitability.text}`,
            '',
            '───────────────────────────────────────────',
            'CAREER PROFILE',
            '───────────────────────────────────────────',
            encouragement || `Your profile shows promise for the ${jobRole} role.`,
            '',
        ];
        ...
        if (missingSkills.length > 0) {
            ...
            lines.push('───────────────────────────────────────────');
            missingSkills.forEach((s) => lines.push(`  • ${s}`));
            lines.push('');
        }
        ...
        if (potentialProblems.length > 0) {
            ...
            lines.push('───────────────────────────────────────────');
            potentialProblems.forEach((s, i) => lines.push(`  ${i + 1}. ${s}`));
            lines.push('');
        }
    ...
    /** Download the analysis summary as a text file */
    const handleDownload = () => {
        ...
        a.href = url;
        a.download = `resume-analysis-${fileName.replace(/\.[^.]+$/, '')}.txt`;
    ...
    const handleShare = async () => {
        ...
        if (navigator.share) {
            try {
                await navigator.share({
                    title: `Resume Analysis — ${fileName}`,
                    text,
                });
                return;
            } catch {
        ...
        try {
            ...
            // Last resort
            alert('Unable to share. Please copy the summary manually.');
        }
    };
            ...
            >
                <div className="container py-4 md:py-5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Button variant="secondary" size="sm" className="hidden md:flex" onClick={handleDownload}>
